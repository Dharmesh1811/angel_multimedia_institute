...
export default function InquiryPopup() {
  ...
  const onSubmit = async (data) => {
    ...
    const eventId = Date.now(); // 🔥 unique ID for tracking
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 px-5 py-4 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-lg font-bold text-white">
              Get Your Free Spoken English Course Now
            </h1>
          </div>
          <button
            onClick={() => setIsOpen(false)}
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-6 rounded-xl mt-4"
              >
                {isSubmitting ? (
                  <>
